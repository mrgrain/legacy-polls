...
(
    ...
    if ($poll_arr['poll_end'] > $date && $voted == false) {
        if ($poll_arr['poll_type'] == 0) {
            ...
            $FD->db()->conn()->exec('UPDATE ' . $FD->db()->getPrefix() . "poll_answers SET answer_count = answer_count + 1 WHERE answer_id = '" . $_POST['answer'] . "'");
            if ($_POST['answer'] != 0) {
            ...
            $id = $id['value'];
            settype($id, 'integer');
            $FD->db()->conn()->exec('UPDATE ' . $FD->db()->getPrefix() . "poll_answers SET answer_count = answer_count + 1 WHERE answer_id = '$id'");
